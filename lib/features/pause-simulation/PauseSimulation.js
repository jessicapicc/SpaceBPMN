...
var PLAY_MARKUP = '<i class="fa fa-play"></i> Play',
    PAUSE_MARKUP = '<i class="fa fa-pause"></i> Pause';
...
if(is(parent, 'bpmn:Participant') || is(parent, 'bpmn:Collaboration') || !parent.parent){
  //  if (!parent.parent) {
      self.activate();
      self.unpause();
...
PauseSimulation.prototype._init = function() {
  this.paletteEntry = domify(
    '<div id="anim-play" title="Play/Pause Animation">' +
      PLAY_MARKUP +
    '</div>'
  );
