...
var events = require('../../util/EventHelper'), TOGGLE_MODE_EVENT = events.TOGGLE_MODE_EVENT,
...
RESET_SIMULATION_EVENT = events.RESET_SIMULATION_EVENT, TERMINATE_EVENT = events.TERMINATE_EVENT, PROCESS_INSTANCE_SHOWN_EVENT = events.PROCESS_INSTANCE_SHOWN_EVENT;

var OFFSET_BOTTOM = 10, OFFSET_LEFT = -15;
...
function TokenCount(eventBus, overlays, elementRegistry, canvas,
	...
	eventBus.on(RESET_SIMULATION_EVENT, function() {
		self.removeTokenCounts();
		self.removeMessageTokenCounts();
	});

	eventBus.on(TERMINATE_EVENT, function(context) {
		var element = context.element, parent = element.parent;
		//self.removeMessageTokenCounts(parent);
		self.removeTokenCounts(parent);
	});

	eventBus.on([ GENERATE_TOKEN_EVENT, CONSUME_TOKEN_EVENT ], LOW_PRIORITY,
			function(context) {
				var element = context.element, parent = element.parent;
				self.removeTokenCounts(parent);
	...
	eventBus.on([ GENERATE_MESSAGE_TOKEN_EVENT, CONSUME_MESSAGE_TOKEN_EVENT ],
			LOW_PRIORITY, function(context) {
				var element = context.element, parent = element.parent;
				self.removeMessageTokenCounts(parent);
				self.addMessageTokenCounts(parent);
			});
...
TokenCount.prototype.addTokenCounts = function(parent) {
	...
	if (!shownProcessInstance) {
		var processInstancesWithParent = this._processInstances
...
TokenCount.prototype.addMessageTokenCounts = function(parent) {
	var self = this;
	...
	this._elementRegistry.forEach(function(element) {
		if (isAncestor(parent, element)) {
			self.addMessageTokenCount(element, shownProcessInstance);
		}
	});
};

TokenCount.prototype.addTokenCount = function(element, shownProcessInstance) {
	var tokenCount = element.tokenCount
			&& element.tokenCount[shownProcessInstance];
...
TokenCount.prototype.addMessageTokenCount = function(element,
		shownProcessInstance) {
	var messageTokenCount = element.messageTokenCount
			&& element.messageTokenCount[shownProcessInstance];

	if (!messageTokenCount) {
		return;
	}

	var html = this.createMessageTokenCount(messageTokenCount);
	...
	var overlayId = this._overlays.add(element, 'message-token-count', {
		position : position,
		html : html,
		show : {
			minZoom : 0.5
...
TokenCount.prototype.createMessageTokenCount = function(messageTokenCount) {
	return domify('<div class="message-token-count waiting">' + messageTokenCount + '</div>');
};
...
TokenCount.prototype.removeMessageTokenCounts = function(parent) {
	var self = this;
	...
	this._elementRegistry.forEach(function(element) {
		if (isAncestor(parent, element)) {
			self.removeMessageTokenCount(element);
		}
	});
};
...
TokenCount.prototype.removeMessageTokenCount = function(element) {
	var overlayId = this.overlayIds[element.id];
