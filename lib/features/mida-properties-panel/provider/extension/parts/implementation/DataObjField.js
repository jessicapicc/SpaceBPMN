...
function generatePropertyId() {
  return utils.nextId('DataField_');
}
...
function getPropertiesElementInsideExtensionElements(extensionElements) {
  return find(extensionElements.values, function(elem) {
    return is(elem, 'mida:DataObjFields');
  });
}
...
module.exports = function(element, bpmnFactory, options, translate) {
  ...
  assign(options, {
    addLabel: translate('Add Data Field'),
    ...
    addElement: function(element, node) {
      ...
      if (!properties) {
        properties = elementHelper.createElement('mida:DataObjFields', {}, parent, bpmnFactory);

        if (!isExtensionElements(parent)) {
          commands.push(cmdHelper.updateBusinessObject(element, parent, { 'dataObjFields': properties }));
        } else {
      ...
      var property = elementHelper.createElement('mida:DataField', propertyProps, properties, bpmnFactory);
      ...
      if (propertyValues.length === 1) {
        ...
        if (!isExtensionElements(parent)) {
          ...
          forEach(parent.values, function(value) {
            if (is(value, 'mida:DataObjFields')) {
